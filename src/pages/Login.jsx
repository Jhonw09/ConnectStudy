  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      if (isLogin) {
        ...
        if (!savedEmail) {
          setAlert({ message: 'Nenhuma conta encontrada. Cadastre-se primeiro.', type: 'error' })
          return
        }
        if (formData.email !== savedEmail) {
          setAlert({ message: 'E-mail incorreto. Verifique e tente novamente.', type: 'error' })
          return
        }
        if (savedPassword && formData.password !== savedPassword) {
          setAlert({ message: 'Senha incorreta. Verifique e tente novamente.', type: 'error' })
          return
        }
        if (formData.name && savedName && formData.name.toLowerCase() !== savedName.toLowerCase()) {
          setAlert({ message: 'Nome incorreto. Verifique e tente novamente.', type: 'error' })
          return
        }
        ...
        if (existingEmail && formData.email === existingEmail) {
          setAlert({ message: 'Já existe uma conta com este e-mail. Faça login.', type: 'error' })
          return
        }
